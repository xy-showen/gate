...
innerServer.on('connection', function(gameServer) {
	...
	gameServer.on('message', function(data) {
		...
		if(JSONdata.url=="/user/updateSession"){//gameServer处理了在别的gameServer玩家的事件之后来让网关通知玩家所在服务器更新session
			if(!global.clients.hasOwnProperty(JSONdata.data.uid)||!global.clients[JSONdata.uid].logined){
				return;
			}else{
			 global.gameServers[global.clients[JSONdata.uid].innerServerIP].gameServer.send(data);
			return;
			}
		}
