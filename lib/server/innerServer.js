...
innerServer.on('connection', function(gameServer) {
	...
	gameServer.on('message', function(data) {
		...
		if (JSONdata.url == "/account/login") { //登录返回的信息
			console.log(JSONdata.data.info.user.uName + " logged succ");
			//console.log(JSONdata.data.info.user._id);
			global.tempClients[JSONdata.data.info.user.uName].logged = true;//更新session状态,添加_id信息
			global.tempClients[JSONdata.data.info.user.uName]._id = JSONdata.data.info.user._id;

			if (!global.clients.hasOwnProperty(JSONdata.data.info.user.uName)) { //把客户端session放到cilents中
				global.clients[JSONdata.data.info.user._id] = global.tempClients[JSONdata.data.info.user.uName];
				global.clients[JSONdata.data.info.user._id].connection.send(data);
				//global.tempClients[JSONdata.data.info.user.uName] = null;
			}
		}
