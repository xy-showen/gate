	...
	gameServer.on('message', function(data) {
		...
		if (JSONdata.url == "/account/login") { //登录返回的信息
			...
			if (!global.clients.hasOwnProperty(JSONdata.username)) { //把链接放到cilents中
				global.clients[JSONdata.data.info.user._id] = global.tempClients[JSONdata.username];
		...
		//console.log(JSONdata._id+"99999999");
		if (JSONdata._id&&global.clients.hasOwnProperty(JSONdata._id.toString())) { //消息发给客户端
